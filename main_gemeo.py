...
class RegistroDeSimulacao:
    def __init__(self, duracao_s, passo_simulacao):
        ...
        self.correntes_carga = np.zeros(self.n_sim)
...
def plotar_graficos_analise_final(registros, nome_ensaio=f"simu_gemeo_uxi_curva_original_{PERFIL_VENTO}"):
    s = registros.idx_sim
    ...
    axs2[1].plot(t, registros.correntes_carga[:s], color='red', lw=2, label='Corrente da Armadura (I_a)')
...
while (tempo_decorrido) < TEMPO_TOTAL_EMULACAO_SEGUNDOS:
    ...
    if (tempo_decorrido) >= PASSO_SIMULACAO_SEGUNDOS: 
        ...
        if fem >= TENSAO_MINIMA_INVERSOR_V:
            ...
            if tempo_acumulado_acima_minimo >= TEMPO_ESPERA_LIGAR:
                if not inversor_conectado:
                    print(f"[{tempo_decorrido:.2f}s] Inversor ligou após permanecer acima de {TENSAO_MINIMA_INVERSOR_V}V por {TEMPO_ESPERA_LIGAR}s!")
                inversor_conectado = True
        else:
            if inversor_conectado:
                print(f"[{tempo_decorrido:.2f}s] Inversor desligou! Tensão caiu abaixo de {TENSAO_MINIMA_INVERSOR_V}V.")
        ...
        if registros.idx_sim < registros.n_sim:
            ...
            registros.correntes_carga[registros.idx_sim] = turbina_gemeo_digital.get_corrente_armadura_amperes()
...
print(f"\n>> [LOG ENERGIA] Total entregue para a curva UxI original: {energia_wh:.4f} Wh ({energia_joules:.2f} J)")
